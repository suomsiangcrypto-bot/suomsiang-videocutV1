...
ipcMain.handle('native-export', function(event, data) {
  ...
  function encodeNext() {
    ...
    var noOrigAudio = (job.isImage || job.muted);
    var args = [];
    ...
    // INPUT 1 = silent audio (เมื่อ image หรือ muted) เพื่อให้ทุก segment มี audio track เท่ากัน
    if (noOrigAudio) {
    ...
    // mapping: ใช้ silent audio (input 1) ถ้า muted/image, ไม่งั้นใช้เสียงจาก video (input 0)
    if (noOrigAudio) {
    ...
    return runFFmpeg(ffmpegPath, args).then(function() {
  ...
  return encodeNext().then(function() {
    ...
    return runFFmpeg(ffmpegPath, [
      ...
      '-map', '0:v', '-map', '[aout]',
      '-c:v', 'copy', '-c:a', 'aac', '-b:a', '192k', '-shortest', '-y', mixOut
    ...
    return runFFmpeg(ffmpegPath, [
      '-i', currentFile, '-i', wavePng,
      '-filter_complex', '[1:v]colorkey=0x000000:0.15:0.1[wk];[0:v][wk]overlay=0:0:shortest=1[vout]',
      '-map', '[vout]', '-map', '0:a?',
      '-c:v', 'libx264', '-crf', String(crf), '-preset', 'fast', '-pix_fmt', 'yuv420p',
      '-c:a', 'copy', '-y', waveOut
